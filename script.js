...
let appData = {};
let order = { page: null, telegramValue: null, customer: {}, products: [], shipping: {}, payment: {}, telegram: {}, subtotal: 0, grandTotal: 0 };
...
let profileMenuButton, profileDropdown, editProfileBtn, logoutBtn, editProfileModal, closeProfileModalBtn, cancelEditProfileBtn, editProfileForm, scannerContainer, closeScannerBtn, imagePreviewModal, previewImage;
...
function buildAppUI() {
    appContainer.innerHTML = `
        ...
        <div id="customerPage" class="hidden w-full max-w-2xl mx-auto"><div class="page-card"><h2 class="text-2xl font-bold mb-6 text-white">១. ព័ត៌មានអតិថិជន</h2><div class="space-y-4">
            ...
            <input type="tel" id="customer-phone" placeholder="លេខទូរស័ព្ទ" class="form-input w-full">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4"><select id="province" class="form-select w-full"></select><select id="district" class="form-select w-full"></select></div>
            ...
            <input type="text" id="additional-location" placeholder="ផ្ទះលេខ, ផ្លូវ, សង្កាត់,..." class="form-input w-full hidden">
            <div><label class="block text-sm font-medium text-gray-400 mb-2">ថ្លៃសេវាដឹកជញ្ជូន</label><div class="flex flex-col sm:flex-row sm:space-x-4 space-y-2 sm:space-y-0"><button id="shipping-fee-btn" class="btn btn-primary flex-1">គិតថ្លៃសេវា</button><button id="no-shipping-fee-btn" class="btn btn-secondary flex-1 bg-red-800 hover:bg-red-700">មិនគិតថ្លៃសេវា</button></div><input type="number" id="shipping-fee-amount" placeholder="តម្លៃសេវាដឹក (ឧ. 1.5)" class="form-input w-full mt-3"></div>
        ...
        <div id="shippingPage" class="hidden w-full max-w-2xl mx-auto"><div class="page-card"><h2 class="text-2xl font-bold mb-6 text-white">៤. វិធីសាស្រ្តដឹកជញ្ជូន</h2><div class="space-y-4">
            ...
            <div class="space-y-2 mt-4">
                 <div class="flex items-center justify-between">
                    <label for="internal-shipping-cost" class="block text-sm font-medium text-gray-400">តម្លៃសេវាដឹក (ចំណាយ)</label>
                    <div class="flex items-center">
                        <input type="checkbox" id="no-internal-shipping-cost" class="h-4 w-4 rounded bg-gray-700 border-gray-600 text-blue-600 focus:ring-blue-500"><label for="no-internal-shipping-cost" class="ml-2 text-gray-300 text-sm">មិនគិតថ្លៃ</label>
                    </div>
                </div>
                <input type="number" id="internal-shipping-cost" placeholder="0.00" class="form-input w-full">
            </div>
    ...
    document.getElementById('no-internal-shipping-cost').addEventListener('change', (e) => {
        const costInput = document.getElementById('internal-shipping-cost');
        if (e.target.checked) {
            costInput.value = 0;
            costInput.disabled = true;
            costInput.classList.add('bg-gray-800', 'cursor-not-allowed');
        } else {
            costInput.value = '';
            costInput.disabled = false;
            costInput.classList.remove('bg-gray-800', 'cursor-not-allowed');
        }
    });
}
...
function attachModalListeners() {
    ...
    // Re-assign other modal elements
...
// All other functions from the original script go here...
// (handleLogin, updateProfileDisplay, togglePasswordVisibility, logout, openEditProfileModal, etc.)
...
function updateProfileDisplay() {
    ...
    document.getElementById('user-role').textContent = currentUser.Role || 'N/A';
...
async function handleProfileUpdateSubmit(e) {
    ...
    try {
        ...
        await fetchData(true); // Force a full data refresh
...
function populateStaticDropdowns() {
    ...
    const provinces = [...new Set(appData.locations.map(item => item.Province))];
    ...
    provinceSelect.addEventListener('change', () => {
        ...
        const districts = appData.locations.filter(l => l.Province === selectedProvince).map(l => l.District);
        districtSelect.innerHTML = '<option value="">-- ជ្រើសរើស ស្រុក/ខណ្ឌ (អាចរំលងបាន) --</option>' + districts.map(d => `<option value="${d}">${d}</option>`).join('');
    });
    ...
    if(document.getElementById('bank-account-select')){
...
function setShippingFeeMode(isFeeEnabled) {
    ...
    if (isFeeEnabled) {
        document.getElementById('shipping-fee-btn').classList.replace('btn-secondary', 'btn-primary');
        document.getElementById('no-shipping-fee-btn').classList.replace('btn-primary', 'btn-secondary');
        shippingFeeInput.classList.remove('hidden');
    } else {
        document.getElementById('no-shipping-fee-btn').classList.replace('btn-secondary', 'btn-primary');
        document.getElementById('shipping-fee-btn').classList.replace('btn-primary', 'btn-secondary');
...
function validateAndGoToProducts() {
    ...
    order.customer.shippingFee = isNaN(fee) || fee < 0 || feeInput.classList.contains('hidden') ? 0 : fee;
...
// All other functions from the original script go here...
// (renumberProductForms, addProduct, createProductForm, attachProductFormListeners, etc.)
...
function goToFinalConfirmation() {
    ...
    if (costInput.value.trim() === '') {
        alert('សូមបញ្ចូលតម្លៃសេវាដឹក ឬធីក "មិនគិតថ្លៃ"។');
        return;
    }

    const cost = parseFloat(costInput.value);
    order.shipping.cost = isNaN(cost) || cost < 0 ? 0 : cost;
...
function renderFinalConfirmationPage() {
    ...
    document.getElementById('final-customer-location').textContent = `${order.customer.province}, ${order.customer.district || ''}`;
...
async function handleSubmitOrder() {
    ...
    order.telegram.schedule = document.getElementById('schedule-telegram').checked;
    if(order.telegram.schedule){
        const localTime = document.getElementById('telegram-schedule-time').value;
        // Convert local datetime-local value to UTC ISO string
        order.telegram.time = new Date(localTime).toISOString();
    } else {
        order.telegram.time = null;
    }
    ...
    try {
        await fetch(WEB_APP_URL, { method: 'POST', mode: 'no-cors', cache: 'no-cache', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(orderData) });
...
function resetApp() {
    ...
    order = { page: null, telegramValue: null, customer: {}, products: [], shipping: {}, payment: {}, telegram: {}, subtotal: 0, grandTotal: 0 };
...
function updateProductSuggestions() {
    const productDatalist = document.getElementById('product-suggestions') || document.createElement('datalist');
    productDatalist.id = 'product-suggestions';
